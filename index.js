/* User Persona WorldForge for SillyTavern - v0.5.1 */
const EXT = 'user-persona-worldforge', VERSION = '0.5.1';

const DEFAULTS = {
  mode: 'standard',
  theme: 'blue', // blue (静谧蓝白), green (鼠尾草绿), purple (暮山灰紫), warm (暖杏米白)
  zoom: 100,     // 缩放百分比: 80, 90, 100, 110, 120
  worldName: '',
  autoWorld: true,
  personaName: '',
  identity: '',
...
function setupWandMenu() {
  const addMenuItem = () => {
    ...
    extMenus.forEach(menu => {
      ...
      item.innerHTML = `
        <i class="fa-solid fa-wand-magic-sparkles fa-fw upw-menu-icon"></i>
        <span>User 人设助手</span>
      `;
      item.addEventListener('click', (e) => {
        e.stopPropagation();
        const parentDropdown = item.closest('.dropdown-menu, .popup, #extensionsMenu');
        if (parentDropdown) $(parentDropdown).hide?.();
        toggleModal(true);
      });
    ...
    // 2. 兼容聊天框旁的魔法棒弹窗
    const wandPopups = document.querySelectorAll('#expression-wrapper, #magic_menu, .expression-list');
    wandPopups.forEach(popup => {
      if (popup.querySelector('#upw-wand-trigger')) return;
      const wandItem = document.createElement('div');
      wandItem.id = 'upw-wand-trigger';
      wandItem.className = 'list-group-item list-group-item-action interactable upw-menu-row';
      wandItem.innerHTML = `
        <i class="fa-solid fa-wand-magic-sparkles fa-fw upw-menu-icon"></i>
        <span>User 人设助手</span>
      `;
      wandItem.addEventListener('click', (e) => {
        e.stopPropagation();
        popup.style.display = 'none';
        toggleModal(true);
      });
      popup.prepend(wandItem);
    });
  };
  ...
  const topExtBtn = document.querySelector('#extensionsMenuButton, #nav-extensions-button, [data-i18n="Extensions"]');
  if (topExtBtn) {
    topExtBtn.addEventListener('click', () => setTimeout(addMenuItem, 80));
  }
}
